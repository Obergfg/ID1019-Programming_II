defmodule Game do
  ...
  def move_ball(player1, player2, ball) do
    ...
    cond do
      dx > 0 and bx >= @field_width - 2 * @paddle_width - @ball_width and
          (by >= y2 - @ball_width and by <= y2 + @paddle_height) ->
        # :io.format(" ball hits right paddle at: ~w ~w\n", [bx, by])
        {dx, dy} = spin(y2, by, dx, dy)

        {:bounce, {bx, by}, {:ball, bx, by, dx, dy}}

      dx < 0 and bx <= 2 * @paddle_width and
          (by >= y1 - @ball_width and by <= y1 + @paddle_height) ->
